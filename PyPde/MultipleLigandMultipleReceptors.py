import numpy as np
import numba as nb
from pde import PDEBase, UnitGrid, CartesianGrid, SphericalGrid, ScalarField, FieldCollection, PlotTracker, FileStorage
...
# We should automate the production rates in a better way
...
eq = LigandReceptorsPDE(diffs=scaled_diffusivities, \
...
plot_tracker = PlotTracker(interval=1, scale='unity') # To plot the solutions at certain time intervals
trackers = ['progress', storage_write.tracker(t_track)] # To save the solutions
sol = eq.solve(initial_data, t_range=t_end, dt = dt, tracker=trackers, method='scipy') # Run the solution

# # Iterate through each of the scaling factors now
# for scale in scaling_factors:

#     # Iterate over scaled diffusivities
#     scaled_diffusivities = [0.001, 0.001]
#     scaled_diffusivities[1] = scaled_diffusivities[1]*scale
...
#                 + '_p_' + str(num_complexes) \
#                 + f'_sameregion_liganddiffs_D1_{scaled_diffusivities[0]:.1E}_D2_{scaled_diffusivities[1]:.1E}' \
#                 + '.h5'

#     # Now solve the PDE
#     # plot_tracker = PlotTracker(interval=0.1, scale='unity') # Plot the solution at every 0.1 steps
#     storage_write = FileStorage(fileName)
#     sol = eq.solve(initial_data, t_range=t_end, dt = dt, tracker=['progress', storage_write.tracker(t_track)], method='scipy')

#     # Iterate over scaled production_rates
#     scaled_prod_scales = [0.0001, 0.0001]
#     scaled_prod_scales[1] = scaled_prod_scales[1]*scale
#     scaled_production_rates = [scaled_prod_scales[0]*ScalarField.from_expression(grid, '0.5 * (tanh(50.0*(y - 40*0.2)) + tanh(50.0*(0.3*40 - y)))'), \
#     scaled_prod_scales[1]*ScalarField.from_expression(grid, '0.5 * (tanh(50*(y - 0.2*40)) + tanh(50*(0.3*40 - y)))')]

#     # Define the PDE model
#     eq = LigandReceptorsPDE(diffs=diffusivities, \
#                             prod_rates=scaled_production_rates, \
#                             bind_rates=binding_rates, \
#                             diss_rates=dissociation_rates, \
#                             degrad_rates=degradation_rates, bc='natural')
...
#     fileName = './Results/LigandsSameRegion/ligandreceptormodel_n_' + str(num_ligands) + '_m_' + str(num_receptors) \
#                 + '_p_' + str(num_complexes) \
#                 + f'_sameregion_ligandprods_p1_{scaled_prod_scales[0]:.1E}_p2_{scaled_prod_scales[1]:.1E}' \
#                 + '.h5'
...
#     # Iterate over the degradation rates
#     scaled_degradation_rates = np.array([0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001])
#     scaled_degradation_rates[1] = scaled_degradation_rates[1]*scale
...
#                             bind_rates=binding_rates, \
#                             diss_rates=dissociation_rates, \
#                             degrad_rates=scaled_degradation_rates, bc='natural')

#     # Define the file name
#     fileName = './Results/LigandsSameRegion/ligandreceptormodel_n_' + str(num_ligands) + '_m_' + str(num_receptors) \
#                 + '_p_' + str(num_complexes) \
#                 + f'_sameregion_liganddegrads_p1_{scaled_degradation_rates[0]:.1E}_p2_{scaled_degradation_rates[1]:.1E}' \
#                 + '.h5'
...
# # Iterate over the binding rates and dissociation rates
# binding_indices = np.nonzero(binding_rates)
# ligand_indices = binding_indices[0]
# # receptor_indices = binding_indices[1]

# # # We should automate the production rates in a better way
# # diffusivities = np.array([0.001, 0.001])
# # prod_scalars = [0.0001, 0.0001]
# # # production_rates = [prod_scalars[0]*ScalarField.from_expression(grid, '0.5 * (tanh(50.0*(y - 40*0.2)) + tanh(50.0*(0.3*40 - y)))'), \
# # #     prod_scalars[1]*ScalarField.from_expression(grid, '0.5 * (tanh(50*(y - 0.7*40)) + tanh(50*(0.8*40 - y)))')]
# # production_rates = [prod_scalars[0]*ScalarField.from_expression(grid, '0.5 * (tanh(50.0*(y - 40*0.2)) + tanh(50.0*(0.3*40 - y)))'), \
# #     prod_scalars[1]*ScalarField.from_expression(grid, '0.5 * (tanh(50*(y - 0.2*40)) + tanh(50*(0.3*40 - y)))')]
...
# #     ligand_index = int(ligand_indices[i])
# #     receptor_index = int(receptor_indices[i])

# #     for scale in scaling_factors:

# #         # Iterate over scaled binding rates
# #         scaled_binding_rates = np.array([[0.1, 0.0, 0.1], [0.1, 0.1, 0.0]])
# #         scaled_binding_rates[ligand_index, receptor_index] = binding_rates[ligand_index, receptor_index]*scale

# #         # Define the PDE model
# #         eq = LigandReceptorsPDE(diffs=diffusivities, \
# #                                 prod_rates=production_rates, \
# #                                 bind_rates=scaled_binding_rates, \
# #                                 diss_rates=dissociation_rates, \
# #                                 degrad_rates=degradation_rates, bc='natural')

# #         # Define the file name
# #         fileName = './Results/LigandsSameRegion/ligandreceptormodel_n_' + str(num_ligands) + '_m_' + str(num_receptors) \
# #                     + '_p_' + str(num_complexes) + '_sameregion_ligandbinds'
                    
# #         for j in range(len(ligand_indices)):
# #             lig = int(ligand_indices[j])
# #             rec = int(receptor_indices[j])
# #             fileName += f'_alph{lig + 1},{rec + 1}_{scaled_binding_rates[lig, rec]:.1E}'
...
# #         # Iterate over scaled production_rates
# #         scaled_dissociation_rates = np.array([[0.001, 0.0, 0.001], [0.001, 0.001, 0.0]])
# #         scaled_dissociation_rates[ligand_index, receptor_index] = dissociation_rates[ligand_index, receptor_index]*scale

# #         # Define the PDE model
# #         eq = LigandReceptorsPDE(diffs=diffusivities, \
# #                                 prod_rates=production_rates, \
# #                                 bind_rates=binding_rates, \
# #                                 diss_rates=scaled_dissociation_rates, \
# #                                 degrad_rates=degradation_rates, bc='natural')

# #         # Define the file name
# #         fileName = './Results/LigandsSameRegion/ligandreceptormodel_n_' + str(num_ligands) + '_m_' + str(num_receptors) \
# #                     + '_p_' + str(num_complexes) + '_sameregion_liganddiss'
                    
# #         for j in range(len(ligand_indices)):
# #             lig = int(ligand_indices[j])
# #             rec = int(receptor_indices[j])
# #             fileName += f'_beta{lig + 1},{rec + 1}_{scaled_dissociation_rates[lig, rec]:.1E}'
